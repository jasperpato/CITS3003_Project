...
void point_source(vec4 Lpos, vec3 lrgb, float b, vec3 distParam) {
    ...
    vec3  specular = Ks * SpecularProduct * b * lrgb; // tends to light source rgb
...
void directional(vec4 Lpos, vec3 lrgb, float b, vec3 distParam) {

    // The vector to the light from the vertex    
    vec3 Lvec = (Lpos - OriginPosition).xyz;

    // Unit direction vectors for Blinn-Phong shading calculation
    vec3 L = normalize( Lvec );   // Direction to the light source
    vec3 H = L;  // Halfway vector
...
void main()
{
    ...
    point_source(LightPosition1, LightRGB1, LightBrightness1, distParams1);

    vec3 distParams2 = vec3(0.0, 0.0, 0.0);
    directional(LightPosition2, LightRGB2, LightBrightness2, distParams2); // light source to origin
