...
static float viewDist = 7; // Distance from the camera to the centre of the scene
static float camRotSidewaysDeg = 0; // rotates the camera sideways around the centre
static float camRotUpAndOverDeg = -40; // rotates the camera up and over the centre.
...
void display(void) {
    ...
    glUniform4fv(glGetUniformLocation(shaderProgram, "LightPosition1"), 1, view * sceneObjs[1].loc);
    glUniform3fv(glGetUniformLocation(shaderProgram, "LightRGB1"), 1, sceneObjs[1].rgb);
    glUniform1f(glGetUniformLocation(shaderProgram, "LightBrightness1"), sceneObjs[1].brightness);
    
    // PART I
    glUniform4fv(glGetUniformLocation(shaderProgram, "LightPosition2"), 1, view * sceneObjs[2].loc);
    glUniform3fv(glGetUniformLocation(shaderProgram, "LightRGB2"), 1, sceneObjs[2].rgb);
    glUniform1f(glGetUniformLocation(shaderProgram, "LightBrightness2"), sceneObjs[2].brightness);
    
    glUniform4fv(glGetUniformLocation(shaderProgram, "OriginPosition"), 1, view * vec4(0.0, 0.0, 0.0, 1.0));
    CheckError();
...
// PART C
static void adjustAmbDif(vec2 af) {
    sceneObjs[toolObj].ambient += af[0];
    sceneObjs[toolObj].diffuse += af[1];
}

// PART C
static void adjustSpecShine(vec2 ss) {
    sceneObjs[toolObj].specular += ss[0];
    sceneObjs[toolObj].shine += min(max(0.0, (double)ss[1]), 100.0); // 0 <= shine <= 100
}
